...
//		private Color[] c = {};
	...
	public ObjLoader(String id, Shader shader, File sourceFile, File sourceTex) {
		super(id, shader);
		if(sourceTex!=null)tex = new Texture(sourceTex);

		if(sourceFile==null)sourceFile=new File("obj/cube.obj");
		List<String> linesInObj = Arrays.asList(BasicFunctions.readFile(sourceFile));
		...
		positionData3 = BufferUtils.createFloatBuffer(vertices3.length * vecmath.vectorSize());
		colorData3 = BufferUtils.createFloatBuffer(vertices3.length * vecmath.colorSize());
		normalData3 = BufferUtils.createFloatBuffer(vertices3.length * vecmath.vectorSize());
		if(tex!=null)textureData3 = BufferUtils.createFloatBuffer(vertices3.length * vecmath.vectorSize());

		for (Vertex v : vertices3) {
			positionData3.put(v.position.asArray());
			colorData3.put(v.color.asArray());
			normalData3.put(v.normal.asArray());
			if(tex!=null)textureData3.put(v.texcoord.asArray());
		}
		positionData3.rewind();
		colorData3.rewind();
		normalData3.rewind();
		if(tex!=null)textureData3.rewind();

		positionData = BufferUtils.createFloatBuffer(vertices.length * vecmath.vectorSize());
		colorData = BufferUtils.createFloatBuffer(vertices.length * vecmath.colorSize());
		normalData = BufferUtils.createFloatBuffer(vertices.length * vecmath.vectorSize());
		if(tex!=null)textureData = BufferUtils.createFloatBuffer(vertices.length * vecmath.vectorSize());

		for (Vertex v : vertices) {
			positionData.put(v.position.asArray());
			colorData.put(v.color.asArray());
			normalData.put(v.normal.asArray());
			if(tex!=null)textureData.put(v.texcoord.asArray());
		}
		positionData.rewind();
		colorData.rewind();
		normalData.rewind();
		if(tex!=null)textureData.rewind();
	}
	
	private Vector[] getVecs(List<String> source, Vector[] points) {
		LinkedList<Vector> targetP = new LinkedList<Vector>(Arrays.asList(points));
		for (String s : source) {
			if (s.startsWith("v ")) {
				String[] num = s.split("\\s+");
				targetP.add(Vertex.vec(Float.parseFloat(num[1]), Float.parseFloat(num[2]), Float.parseFloat(num[3])));
			}
		}
	...
	private Vector[] getNormals(List<String> source, Vector[] normals) {
		LinkedList<Vector> targetP = new LinkedList<Vector>(Arrays.asList(normals));
		for (String s : source) {
			if (s.startsWith("vn ")) {
				String[] num = s.split("\\s+");
				targetP.add(Vertex.vec(Float.parseFloat(num[1]), Float.parseFloat(num[2]), Float.parseFloat(num[3])));
	...
	private Vector[] getTex(List<String> source, Vector[] text) {
		...
		for (String s : source) {
			if (s.startsWith("vt ")) {
				...
				if (num.length == 3)
					targetP.add(Vertex.vec(-1*Float.parseFloat(num[1]), Float.parseFloat(num[2]), 0));
				else if (num.length == 4)
					targetP.add(Vertex.vec(-1*Float.parseFloat(num[1]), Float.parseFloat(num[2]),
	...
	private void fillVert(List<String> source) {
		LinkedList<Vertex> targetV3 = new LinkedList<Vertex>(Arrays.asList(vertices3));
		LinkedList<Vertex> targetV4 = new LinkedList<Vertex>(Arrays.asList(vertices));
		for (String s : source) {
			if (s.startsWith("f ")) {
				String[] num = s.split("\\s+");
				if (num.length == 4) {
...
//					System.out.println("5");
				...
				}*/else /*doATri(targetV3, num);*/System.out.println("Not Supported");
	...
	private void doATri(LinkedList<Vertex> targetV3, String[] lineSplitted) {
		...
		if (v1.normal == null){
			...
			v1.setNormal(v3.position.sub(v2.position).cross(v1.position.sub(v2.position)));
//			v1.setNormal(v1.position.cross(v2.position).cross(v3.position));
		}
		if (v2.normal == null)
			v2.setNormal(v3.position.sub(v2.position).cross(v1.position.sub(v2.position)));
		if (v3.normal == null)
			v3.setNormal(v3.position.sub(v2.position).cross(v1.position.sub(v2.position)));
		targetV3.add(v1);
		targetV3.add(v2);
		targetV3.add(v3);
	}
	...
	private void doAQuad(LinkedList<Vertex> targetV4, String[] lineSplitted) {
		...
		if (v3.normal == null)
			v3.setNormal(v3.position.sub(v2.position).cross(v1.position.sub(v2.position)));
		if (v4.normal == null)
			v4.setNormal(v3.position.sub(v2.position).cross(v1.position.sub(v2.position)));
		targetV4.add(v1);
		targetV4.add(v2);
		targetV4.add(v3);
		targetV4.add(v4);
